...
p. Seam Mail provides a way to replace out the default "send" implementation with one of your own via the "MailTransporter":https://github.com/seam/mail/blob/develop/api/src/main/java/org/jboss/seam/mail/core/MailTransporter.java interface. That way when you call @msg.send()@ you can control how and when the email is "sent". What follows is a basic implementation.

p. _In the code that follows I'm skipping the import statements for the sake of readability, but everything is standard EE6 + Seam 3 with exception of the @Files@ which comes from "Google Guava":http://code.google.com/p/guava-libraries/._

p. The default MailTransporterImpl is very simple and looks as follows.

bc.. public class MailTransporterImpl implements MailTransporter {
...
p. First lets create a new MailTransporter called MailQueueTransporter

bc.. public class MailQueueTransporter implements Serializable, MailTransporter {
...
p. Starting from the top we see that we are injecting a new "Event":http://docs.oracle.com/javaee/6/api/javax/enterprise/event/package-summary.html qualified with @@QueuedMail@ and typed to "MimeMessage":http://docs.oracle.com/javaee/6/api/javax/mail/internet/MimeMessage.html. @@QueuedMail@ is a simple qualifier, and if you have never written one before here is what it looks like.
...
p. Next we inject the javax.mail.Session as managed by Seam Mail and implement the @public EmailMessage send(EmailMessage emailMessage)@ as mandated by the interface. Inside this method we use the Seam MailUtility class to convert our EmailMessage as built by Seam Mail to a standard MimeMessage and then fire it as as a CDI event.

p. A CDI event however is still blocking in its execution so lets see what is observing this event.

bc.. public class MailQueueReceiver {
...
p. All that is needed now is a process to send these stored messages. I'm using a EJB 3.1 scheduled timer, but the options are endless.
...
p. One thing to note is that you have to reset the @javax.mail.Session@ as that is lost when the MimeMessage is converted to a OutputStream. There is a little added complexity going on here as I like to control the domain part of a Message-ID, but this is a simple reliable way to handle sending mail in a fast and reliable fashion.