---
title: "CDI Proxies and JSF aren't Best Friends Yet"
layout: blog
author: Cody Lerum
...
p. The CDI EG has determined that there is not meaningful way to implement <code>.equals()</code> and <code>.hashCode()</code> on a proxy that can delegate to the underlying bean (See "OWB-458":https://issues.apache.org/jira/browse/OWB-458 & "WELD-695":https://issues.jboss.org/browse/WELD-695). What this means is that two proxies of the same underlying bean with be equal. However, two proxies to underlying beans which are equal according to <code>.equals</code> will not be equal.

h5. How can we fix this?

p. One solution would be to execute the <code>.equals()</code> on the converted value if a converter exists. However, the JSF RI "Mojarra":http://javaserverfaces.java.net insists ("JAVASERVERFACES-2393":http://java.net/jira/browse/JAVASERVERFACES-2393) that any proxy must delegate equals, hashCode and toString to the proxied instance per "java.lang.reflect.Proxy":http://docs.oracle.com/javase/6/docs/api/java/lang/reflect/Proxy.html. While correct, I feel that if CDI cannot comply with the standard java.lang.reflect.Proxy contract then the JSF and CDI expert groups need to step in and find a solution so that developers can enjoy consistent and predictable behavior in their applications.
